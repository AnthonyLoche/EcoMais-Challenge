...
import { MapPin, Cpu, AlertTriangle } from "lucide-react";
...
export default function MachineModalLeftPanel({ machine }) {
  ...
  return (
    <div className="w-52 shrink-0 border-r border-gray-100 flex flex-col overflow-y-auto
      [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">

      {/* Imagem */}
      <div className="relative bg-gray-50 flex items-center justify-center h-44 shrink-0">
        {machine.imagem
          ? <img src={machine.imagem} alt={machine.codigo} className="w-full h-full object-contain p-2" />
          : <Cpu size={56} strokeWidth={1} className="text-gray-300" />
        }
        
        {/* Status badge */}
        <span className="absolute top-2.5 right-2.5 flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-full border"
      ...
      <div className="p-4 flex flex-col gap-4">
        <div>
          <p className="text-[10px] text-gray-400 uppercase tracking-wider">ID #{machine.id}</p>
